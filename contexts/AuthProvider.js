...
export default function AuthProvider({children}) {
    ...
    const {state : {apiKey}} = useContext(PreferenceContext)
    ...
    function login({inputMobileNumber}) {
        ...
        setLoginError(false)
        setSmsWaiting(false)
        loginMethod(mobileNumVal,apiKey,setAllAccessTokens,setLoginAttempt,setLoginError,setSmsWaiting,setOtpSubmitting,setMobileNumber)
        /*setTimeout(()=> {
            if(accessToken === null)
                setLoginError(true)
        },3 * 60 * 1000) */
    ...
    const state = {accessToken,loginAttempt,loginError,smsWaiting,otpSubmitting}
 ...
 * @param {React.Dispatch<React.SetStateAction<boolean>>} setOtpSubmitting
 */
async function loginMethod(mobileNumber,apiKey,setAccessToken,setLoginAttempt,setLoginError,setSmsWaiting,setOtpSubmitting,setMobileNumber) {
